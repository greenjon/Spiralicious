package llm.slop.spirals.viewmodel
...
class SpiralsViewModel(
    ...
    private val db: MandalaDatabase,
    ...
    val allPatches = patchDao.getAllPatches().stateIn(coroutineScope, SharingStarted.Eagerly, emptyList())
    val allSets = setDao.getAllSets().stateIn(coroutineScope, SharingStarted.Eagerly, emptyList())
    val allMixerPatches = mixerDao.getAllMixerPatches().stateIn(coroutineScope, SharingStarted.Eagerly, emptyList())
    val allShowPatches = showDao.getAllShowPatches().stateIn(coroutineScope, SharingStarted.Eagerly, emptyList())
    val allRandomSets = randomSetDao.getAllRandomSets().stateIn(coroutineScope, SharingStarted.Eagerly, emptyList())
    ...
    private fun initialStack(): List<NavLayer> {
        ...
        if (mode == llm.slop.spirals.StartupMode.LAST_WORKSPACE) {
            val saved = appConfig.loadNavStack()
            if (!saved.isNullOrEmpty()) return saved
        }

        val type = when (mode) {
            llm.slop.spirals.StartupMode.MIXER -> LayerType.MIXER
            llm.slop.spirals.StartupMode.SET -> LayerType.SET
            llm.slop.spirals.StartupMode.MANDALA -> LayerType.MANDALA
            llm.slop.spirals.StartupMode.SHOW -> LayerType.SHOW
    ...
    private fun saveWorkspaceIfEnabled() {
        if (appConfig.startupMode == llm.slop.spirals.StartupMode.LAST_WORKSPACE) {
            appConfig.saveNavStack(_navStack.value)
        }
    }

    fun getStartupMode(): llm.slop.spirals.StartupMode = appConfig.startupMode
    fun setStartupMode(mode: llm.slop.spirals.StartupMode) {
    ...
    fun deletePatch(name: String) {
        coroutineScope.launch { patchDao.deleteByName(name) }
    }
    ...
    fun deleteSet(id: String) {
        coroutineScope.launch { setDao.deleteById(id) }
    }
    ...
    fun deleteMixerPatch(id: String) {
        coroutineScope.launch { mixerDao.deleteById(id) }
    }
    ...
    fun deleteShowPatch(id: String) {
        coroutineScope.launch { showDao.deleteById(id) }
    }
    ...
    fun deleteRandomSet(id: String) {
        coroutineScope.launch { randomSetDao.deleteById(id) }
    }
