...
class DesktopSpiralRenderer : ISpiralRenderer {
    ...
    override fun onDrawFrame() {
        ...
        visualSource?.update(lfoValues)
        
        // Determine ratio: prefer visualSource recipe if available
        // But if visualSource is set, we might need to cast it to MandalaVisualSource to get recipe
        // VisualSource interface doesn't have 'recipe'.
        // Assuming MandalaVisualSource for now or fallback to currentRatio
        val vsRatio = (visualSource as? llm.slop.spirals.MandalaVisualSource)?.recipe
        val ratio = vsRatio ?: currentRatio ?: return
        
        // Also apply params from visualSource if available (like scale, thickness etc which are in uniforms)
        // Currently applyRatio sets VBOs. Uniforms are set below.
        ...
        // Use values from VisualSource if available
    ...
    private fun createProgram(vsPath: String, fsPath: String): Int {
        val vs = readShaderResource(vsPath)
        val fs = readShaderResource(fsPath)
